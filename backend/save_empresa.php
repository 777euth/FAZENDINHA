...
if (!isset($_SESSION['categoria']) || !in_array($_SESSION['categoria'], ['Admin', 'Cadastrar'])) {
    error_log('Sessão inválida ou categoria não permitida: ' . (isset($_SESSION['categoria']) ? $_SESSION['categoria'] : 'Sessão não definida'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($empresa) || empty($cnpj)) {
        echo json_encode(['status' => 'error', 'message' => 'Empresa e CNPJ são obrigatórios']);
        exit;
    }
    ...
    if (strlen($cnpj) !== 14 || !is_numeric($cnpj)) {
        echo json_encode(['status' => 'error', 'message' => 'CNPJ inválido (deve conter 14 dígitos numéricos)']);
        exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO empresas (empresa, cnpj, site_original, telefone, endereco, cidade, estado, cep, pdf_cnpj, pdf_susep) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssss", $empresa, $cnpj, $site_original, $telefone, $endereco, $cidade, $estado, $cep, $pdf_cnpj_path, $pdf_susep_path);
    ...
    if ($success) {
        echo json_encode([
            ...
            'redirect' => '../pages/dashboard_cadastrar.php'
        ]);
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Erro ao adicionar empresa: ' . $stmt->error]);
    }
